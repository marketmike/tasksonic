...
class project
{
   ...
   function GetprojectByCategory($cdrinfoid,$citycurrent) 
   {
      
      global $db;
	  $sql = "SELECT count(*) cnt FROM ".PROJECT_MASTER." AS PM "
			 ." LEFT JOIN ".MEMBER_MASTER." AS MM ON PM.auth_id_of_post_by = MM.user_auth_id "
	         ." WHERE FIND_IN_SET('$cdrinfoid',PM.project_category) AND (PM.project_status =1 AND PM.project_city ='$citycurrent')";
	  $db->query($sql);
	  $db->next_record();
	  $_SESSION['total_record'] = $db->f("cnt") ;
	  $db->free();
	...
	################################################################################################################
	# Function		: ViewAll_Staged_Task_User_Open($post_by_id)
	# Purpose		: This function is used to get staged task for the user
	# Used At 		: view_all_my_task.php
	################################################################################################################
	function ViewAll_Staged_Task_User_Open($post_by_id,$citycurrent)
	{
	  	 global $db3;
		 $sql = "SELECT count(*) as tskcnt FROM ".PROJECT_STAGING_MASTER
		        ." WHERE auth_id_of_post_by = '".$post_by_id."' AND project_city ='$citycurrent'"
				." ORDER BY project_post_time DESC";
				$db3->query($sql);
				$db3->next_record();
				$_SESSION['total_record'] = $db3->f("tskcnt") ;
				$db3->free();

				$sql = "SELECT * FROM ".PROJECT_STAGING_MASTER
				." WHERE auth_id_of_post_by = '".$post_by_id."' AND project_city ='$citycurrent'"
				." ORDER BY project_post_time DESC";
				$db3->query($sql);
	}	   
 // This function used to save in temporary table while the user makes a deposit to their account. It is used whent the user does not have enough money in their acccount to post the task 
	function Stage_For_Payment($post,$project_post_by,$project_file_1,$project_file_2,$project_file_3,$paypal_id,$en_project_title,$en_project_description,$premium_transaction_id,$urgent_transaction_id,$project_free,$project_location,$project_city,$latitude,$longitude,$citycurrent)
	{
	  if($post['cat_prod'] == '')
		   {
			 $cates = $post['cat_prod'];
	  ...
	  global $db;
	  $sql="INSERT INTO ".PROJECT_STAGING_MASTER
			...
			." project_budget,expense_budget,project_days_open,project_hide,project_allow_bid,premium_project,urgent_project,perimum_transaction_id,urgent_trascation_id,project_status,project_posted_date,project_free,project_post_time,location,city,latitude,longitude,bidding_time,completed_by,completed_time,project_city,project_online) "
			." VALUES ('".$project_post_by."' ,"
			." '".md5($project_post_by)."' ,"
			." '".$paypal_id."' ,"
			." '' ,"
			." '' ,"
			." '".mysql_real_escape_string($en_project_title)."' ,"
			." '".$cates."' ,"
			." '".mysql_real_escape_string($en_project_description)."' ,"
			." '".$project_file_1."' ,"
			." '".$project_file_2."' ,"
			." '".$project_file_3."' ,"
			." '1' ,"
			." '".$post['tasker_expenses']."' ,"			
			." '".$post['dev']."' ,"
			." '".$post['bidding']."' ,"
			." '".(int)$post['project_options1']."' ,"
			." '".(int)$post['project_options2']."' ,"
			." '".(int)$post['premium_project']."' ,"
			." '".(int)$post['urgent_project']."' ,"
			." '".(int)$premium_transaction_id."' ,"
			." '".(int)$urgent_transaction_id."' ,"
			." '1' ,"
			." '".date('Y-m-d')."' ,"
			." '".$project_free."' ,"
			." now() ,"
			." '".mysql_real_escape_string($project_location)."' ,"
			." '".mysql_real_escape_string($project_city)."' ,"			
			." '".$latitude."' ,"
			." '".$longitude."' ,"			
			." '".$post['bidding_time']."' ,"
			." '".$post['completed_by']."' ,"
	...
	function Get_Staged_Task($pro_id)
  	 {
       global $db;
	   $sql= " SELECT * FROM ".PROJECT_STAGING_MASTER." WHERE project_id = '".$pro_id." '";
	   $db->query($sql);
	   return ($db->fetch_object(MYSQL_FETCH_SINGLE));
	 }

	function Delete_Staged_Task($pro_id1)
	{
		global $db,$physical_path;
	    $sql = "SELECT * FROM ".PROJECT_STAGING_MASTER." WHERE project_id=".$pro_id1;
		$db->query($sql);
		...
		$sql="DELETE FROM ".PROJECT_STAGING_MASTER
			." WHERE project_id=".$pro_id1;
		return($db->query($sql));
	}	 

	 
	function Insert($post,$project_post_by,$project_file_1,$project_file_2,$project_file_3,$paypal_id,$en_project_title,$en_project_description,$premium_transaction_id,$urgent_transaction_id,$project_free,$project_location,$project_city,$latitude,$longitude,$citycurrent)
	{
	  if($post['cat_prod'] == '')
		   {
			 $cates = $post['cat_prod'];
	  ...
	  $sql="INSERT INTO ".project_MASTER
			...
			." '".$post['tasker_expenses']."' ,"			
			." '".$post['bidding']."' ,"
			." '".(int)$post['project_hide']."' ,"
			." '".(int)$post['project_allow_bid']."' ,"
			." '".(int)$post['premium_project']."' ,"
			." '".(int)$post['urgent_project']."' ,"
			." '".(int)$premium_transaction_id."' ,"
			." '".(int)$urgent_transaction_id."' ,"
	...
	function Insert_Post_project_Fees($user_id,$login_id,$amount,$en_project_title,$transaction_id,$desc)
	   {
		   $trans_type = "-";
	       global $db;
	...
	function Insert_Withdraw_Funds_Hold($user_id,$login_id,$amount,$title,$transaction_id,$desc)
	   {
		   $trans_type = "*";
	       global $db;
		   $sql="INSERT INTO ".PAYPAL_MASTER
	...
	function Insert_Urgent_project_Fees($user_id,$login_id,$amount,$en_project_title,$transaction_id,$desc)
	   {
		   $trans_type = "-";
	       global $db;
		   $sql="INSERT INTO ".PAYPAL_MASTER
	...
	function Insert_Post_project_Fees_Premium($user_id,$login_id,$amount,$en_project_title,$transaction_id,$desc)
	   {
		   $trans_type = "-";
	       global $db;
		   $sql="INSERT INTO ".PAYPAL_MASTER
	...
	function Update($post,$project_file_1,$project_file_2,$project_file_3)
   {
       ...
       global $db;
	if($post['clear_notices'] == 1){	   
	   $sql= " UPDATE ".project_MASTER
			." SET project_title										=	'".mysql_real_escape_string($post['title'])."' ,"
            ." project_description										=	'".mysql_real_escape_string($post['desc'])."' ," 
			...
			." bidding_time                                        		=   '".$post['bidding_time']."' ,"
			." completed_by                                       		=   '".$post['completed_by']."' ,"
			." completed_time                                        	=   '".$post['completed_time']."' ,"
			." project_city                                       		=   '".$post['project_city']."' ,"			
			." project_status                                           =   '".(int)$post['project_status']."' ,"
			." project_closed_date                                      =   '0000-00-00' ,"
			." notice_interval                                      	=   '0' ,"
			." notice_interval_completed                                =   '0' ,"
			." notice_interval_awarded_expiring                         =   '0' ,"
			." tasker_completed_date                                    =   '000-00-00 00:00:00' ,"
			." tasker_reimburse                                      	=   '0' ,"
			." tasker_reimburse_desc                                    =   '' ,"
			." task_owner_dispute                                     	=   '0' ,"
			." task_owner_dispute                                     	=   '0' ,"	
			." task_owner_dispute_date                                	=   '0000-00-00 00:00:00' ,"
			." task_owner_dispute_closed_date                         	=   '0000-00-00 00:00:00' ,"
			." task_owner_dispute_reason                         		=   '' ,"						
			." project_hide                                             =   '".(int)$post['project_options1']."' ,"
			." project_allow_bid                                        =   '".(int)$post['project_options2']."' ,"
			." premium_project                                       	=   '".(int)$post['premium_project']."' ,"
	...
	function Insert_Edit_project($post,$file,$en_edit_proj)
	   {
		   ...
		   $sql="INSERT INTO ".project_EDIT_MASTER
					...
					." '".mysql_real_escape_string($en_edit_proj)."' ,"
					." '".$file."' ,"
					." '".date('m/d/y \a\t H:i T')."' "
					." )";
		   $db->query($sql);	
	...
	function Update_task_bidding_days($post, $total_days)
	{
		global $db;
	...
	function Update_task_complete_days($post, $total_days)
	{
		global $db;

		$sql = " UPDATE ". project_MASTER
			.  " SET completed_by  	= '". $total_days. "' "
			.  " WHERE project_id 	= '".$post['project_id']. "' ";
		$db->query($sql);
		return ($db->affected_rows());
	...
	/////////////////////////////////////////
		...
		function InsertBid($post,$user_name,$en_bid_desc)
	   {
		   global $db;
		   $sql="INSERT INTO ".BID_MASTER
					...
					." VALUES ('".$post['project_id']."' ,"
					." '".$user_name."' ,"
					." '".mysql_real_escape_string($post['bid_amount'])."' ,"
					." '".$post['delivery_within']."' ,"
					." '".mysql_real_escape_string($en_bid_desc)."' ,"
					." '".(int)$post['notification_status']."' ,"
					." 1 ,"
					." 1 ,"
					." 1 ,"
					." '".date('Y-m-d')."' ,"
		...
		function Get_Bid_Current_User($pro_id,$user_name) // Get the bid for the current user
		{
			global $db2;
			$sql= " SELECT * FROM ".BID_MASTER ." AS BM "
				  ." LEFT JOIN ".MEMBER_MASTER." AS MM ON BM.bid_by_user = MM.user_login_id "
				  ." LEFT JOIN ".COUNTRY_MASTER." AS CM ON MM.mem_country = CM.country_id "
				  ." WHERE BM.project_id = '".$pro_id." ' AND BM.bid_by_user = '".$user_name." ' AND BM.bid_status = 1";
		   $db2->query($sql);
		   return ($db2->fetch_object(MYSQL_FETCH_SINGLE));
	    }			
        ...
        function Get_Bid_Count_By_Id($pro_id)
		{
			global $db;
		...
		function UpdateBid($post,$user_name,$en_bid_desc)
		{
			global $db;
	
			$sql = " UPDATE ". BID_MASTER
	    		  ." SET bid_amount  	 ='".mysql_real_escape_string($post['bid_amount'])."' ,"
				  ." delivery_within 	 ='".$post['delivery_within']."' ,"
				  ." bid_desc	 	 	 ='".mysql_real_escape_string($en_bid_desc)."',"
				  ." notification_alert  ='".(int)$post['notification_status']."' ,"
				  ." bid_status          ='1',"
  				  ." date	 			 ='".date('Y-m-d')."' ,"
	...
	function Insert_Msg($post,$user_id,$user_name,$en_message_desc)
	   {
		   ...
		   $sql="INSERT INTO ".MESSAGE_BOARD_MASTER
					...
					." '".mysql_real_escape_string($en_message_desc)."' ,"
					." '".date('m/d/y \a\t H:i T')."' "
					." )";
		   $db->query($sql);	
		}
	...
	################################################################################################################
	# Function		: checkProjectStatus($task_id)
	# Purpose		: Run a check on the Tasks status to make sure it is 6 or 7 otherwise we do not allow delete
	################################################################################################################	
	
	function checkProjectStatus($task_id){
		global $db1;
		$sql = "SELECT * FROM ".PROJECT_MASTER." WHERE project_id =  '". $task_id . "'  AND (project_status = 4 OR project_status = 6 OR project_status = 7)";
		$db1->query($sql);
	...
	################################################################################################################
	# Function		: ViewAll_projects_by_city()
	# Purpose		: This function is used for getting project listing
	# Return		: count value
	# Used At 		: all_projects.php
	################################################################################################################

	function ViewAll_projects_by_city($citycurrent)
	{
		global $db;
		$sql="SELECT count(*) as cnt FROM ".PROJECT_MASTER." AS PM "
		     ." LEFT JOIN ".MEMBER_MASTER." AS MM ON PM.auth_id_of_post_by = MM.user_auth_id "
			 ." WHERE PM.project_status =1 AND PM.project_city ='$citycurrent'";
	...
	################################################################################################################
	# Function		: ViewAll_projects_by_city_completed()
	# Purpose		: This function is used for getting project listing
	# Return		: count value
	# Used At 		: completed_tasks.php
	################################################################################################################

	function ViewAll_projects_by_city_completed($citycurrent)
	{
		global $db;
		$sql="SELECT count(*) as cnt FROM ".PROJECT_MASTER." AS PM "
		     ." LEFT JOIN ".MEMBER_MASTER." AS MM ON PM.auth_id_of_post_by = MM.user_auth_id "
			 ." WHERE PM.project_status =6 AND PM.project_city ='$citycurrent'";
	...
	function ViewAll_Online_Latest()
	{
		global $db;
		$sql="SELECT count(*) as cnt FROM ".PROJECT_MASTER." AS PM "
		     ." LEFT JOIN ".MEMBER_MASTER." AS MM ON PM.auth_id_of_post_by = MM.user_auth_id "
			 ." WHERE PM.project_status =1 AND PM.project_online =1 ";
	...
	# Purpose		: This function is used to get the open task for the users task_owner profile
	# Return		: none
	# Used At 		: viewtask_ownerprofile.php
	# Parameters	
	# 1. $post_by_id =>  user id
	################################################################################################################
	function ViewAll_project_User_Open($post_by_id,$citycurrent)
	{
	  	 global $db3;
		 $sql = "SELECT count(*) as tskcnt FROM ".project_MASTER
		        ." WHERE auth_id_of_post_by = '".$post_by_id."' AND project_status = 1 AND project_city ='$citycurrent'"
	...
	# Return		: none
	# Used At 		: search_tasks.php
	# Parameters	
	# 1. $keywords => search keyword
	# 2. $options => search option
	################################################################################################################
	function Search_projects($keywords, $citycurrent)
	{
		global $db;
...
*/			
		...
		$sql="SELECT count(*) as cnt FROM ".project_MASTER." AS PM " 
			." LEFT JOIN ".MEMBER_MASTER." AS MM ON PM.auth_id_of_post_by = MM.user_auth_id "
			. $sql_where
			." AND (project_city ='$citycurrent' AND project_status =1)";
		...
		$sql = "SELECT * FROM ".project_MASTER." AS PM " 
			." LEFT JOIN ".MEMBER_MASTER." AS MM ON PM.auth_id_of_post_by = MM.user_auth_id "
			  . $sql_where." AND (project_city ='$citycurrent' AND project_status =1) "
		      ." ORDER BY premium_project DESC,project_post_time DESC"
			  ." LIMIT ". $_SESSION['start_record']. ", ". $_SESSION['user_page_size'];
		$db->query($sql);
	}
	...
	/////////////////////////////////////////
	function Bid_details_Of_User($project_id,$auth_id_of_post_to)
	{
		global $db;
	...
	function Commission_Paid_By_Task_Owner($project_id,$auth_id_of_post_by)
	{
		global $db;
		$sql="SELECT * FROM ".PAYPAL_MASTER
			." WHERE task_id = '".$project_id."' AND trans_group = 'commission' AND user_auth_id = '".$auth_id_of_post_by."'";
		
		$db->query($sql);
		$db->next_record();
		return($db->f('amount'));
	}
	function Commission_Paid_By_Tasker($project_id,$auth_id_of_post_to)
	{
		global $db;
		$sql="SELECT * FROM ".PAYPAL_MASTER
			." WHERE task_id = '".$project_id."' AND trans_group = 'commission' AND user_auth_id = '".$auth_id_of_post_to."'";
    ...
    function Insert_Seller_Paypal($user_id,$login_id,$amount,$project_title,$transaction_id)
	{
	    $trans_type = "-";
		$desc = "Fees Taken From Task Sonic For Accepting Task Awarding: $project_title";   
		global $db;
		$sql="INSERT INTO ".PAYPAL_MASTER
					." (user_auth_id,user_login_id,amount,status,description,trans_type,transaction_id,trans_group,task_id,date ) "
					." VALUES ('".$user_id."' ,"
					." '".$login_id."' ,"
					." '".$amount."' ,"
					." 1 ,"
     ...
     function Insert_Referral_Comm_Record($user_id,$login_id,$amount,$transaction_id)
	{
	    $trans_type = "+";
		$desc = "Fees Paid For Successful Referral";   
		global $db;
		$sql="INSERT INTO ".PAYPAL_MASTER
    ...
    function Insert_Task_Owner_Paypal($user_id1,$login_id1,$amount1,$project_title,$transaction_id,$trans_group,$task_id)
	{
	    $trans_type = "-";
		$desc = "Fees Charged by Task Sonic For Awarded and Accepted Task: $project_title";   
		global $db;
		$sql="INSERT INTO ".PAYPAL_MASTER
    ...
    function Return_Buyer_Paypal($user_id2,$login_id2,$amount2,$project_title,$transaction_id)
	{
	    $trans_type = "+";
		$desc = "Task Posting Deposit Returned For Awarded/Accepted Task:<br><strong>".$project_title."</strong>";   
		global $db;
		$sql="INSERT INTO ".PAYPAL_MASTER
					." (user_auth_id,user_login_id,amount,status,description,trans_type,transaction_id,date ) "
					." VALUES ('".$user_id2."' ,"
    ...
    function Return_Commission($user_id,$login_id,$amount,$project_title,$transaction_id,$trans_group,$task_id)
	{
	    $trans_type = "+";
		$desc = "Fees Returned by Task Sonic For Incomplete Task:<br><strong>".$project_title."</strong>";   
		global $db;
		$sql="INSERT INTO ".PAYPAL_MASTER
					." (user_auth_id,user_login_id,amount,status,description,trans_type,transaction_id,trans_group,task_id,date ) "
					." VALUES ('".$user_id."' ,"
   	...
   	function Accept_project($project_id)
	{
   	...
   	function Marked_Completed($project_id,$tasker_completed,$tasker_reimburse,$tasker_reimburse_desc)
	{
			global $db;
	   $sql= " UPDATE ".PROJECT_MASTER
			." SET tasker_completed							=	'".(int)$tasker_completed."' ,"
			." tasker_reimburse                             =   '".mysql_real_escape_string($tasker_reimburse)."' ,"
			." tasker_reimburse_desc                        =   '".mysql_real_escape_string($tasker_reimburse_desc)."' ,"
			." project_status                       		=   '5' ,"			
			." tasker_completed_date                        =   '".date('Y-m-d H:i:s')."' "
			." WHERE project_id = '".$project_id." '";
			$db->query($sql);
	}
   	function Dispute_Completed($project_id,$task_owner_dispute,$task_owner_dispute_reason)
	{
			global $db;
	   $sql= " UPDATE ".PROJECT_MASTER
			." SET task_owner_dispute							=	'".(int)$task_owner_dispute."' ,"
			." task_owner_dispute_reason                        =   '".mysql_real_escape_string($task_owner_dispute_reason)."' ,"		
			." task_owner_dispute_date                       	=   '".date('Y-m-d H:i:s')."' "
			." WHERE project_id = '".$project_id." '";
			$db->query($sql);
	}
   	function Update_Dispute($project_id,$task_owner_dispute)
	{
			global $db;
	   $sql= " UPDATE ".PROJECT_MASTER
			." SET task_owner_dispute							=	'".(int)$task_owner_dispute."' ,"		
			." task_owner_dispute_closed_date                   =   '".date('Y-m-d H:i:s')."' "
			." WHERE project_id = '".$project_id." '";
			$db->query($sql);
	}	
	function Insert_dispute_comment($project_id,$posted_by,$disp_comment)
	{
		global $db;
		$sql="INSERT INTO ".TASK_DISPUTE
					." (task_id,posted_by,disp_comment,date) "
					." VALUES ('".$project_id."' ,"
					." '".$posted_by."' ,"
					." '".mysql_real_escape_string($disp_comment)."' ,"
					." '".date('Y-m-d H:i:s')."' "					
					." )";
     	$db->query($sql);	
	}
	function Getdisputethread($pro_id)
  	 {
		  global $db;
		  $sql = "SELECT * FROM ".TASK_DISPUTE
		  		." WHERE task_id = '".$pro_id."' ORDER BY date DESC"; 
		  $db->query($sql);
	 }
	function Get_Task_Escrow($project_id)
	{
		global $db;
	...
	function Check_If_Escrow_Created($project_id)
	{
		global $db;
		$sql="SELECT * FROM ".ESCROW_PAYMENT_MASTER
			." WHERE project_id = '".$project_id."' AND payment_type = 2";
		$db->query($sql);
		$db->next_record();
		return($db->f('amount'));
	}	
	// Update Escrow Payment Type If No Reimburse On Task Marked Completed
	function update_Escrow_Payment_Type($project_id)
	{
		global $db;
	...
	function notify_re_award($project_id){
		global $db;
		 $sql= " SELECT * FROM ".PROJECT_MASTER." AS PM "
	...
	function GetprojectByCategory123($cdrinfoid) 
   {
	...
	function Insert_Referrals_Paid($project_id,$amount_paid,$paid_to)
	   {
		   global $db;
		   $sql="INSERT INTO ".REFERRALS_PAID
					." (project_id,amount_paid,paid_to,submitted_date) "
					." VALUES ('".$project_id."' ,"
					." '".$amount_paid."' ,"
					." '".$paid_to."' ,"					
					." '".date('Y-m-d')."' "
	...
	function Insert_Commission($project_id,$buyer_comm,$seller_comm)
	   {
		   global $db;
	...
	function UpdateMessage($post)
	{
		...
		$sql= " UPDATE ".MESSAGE_BOARD_MASTER
			." SET "
			." message_desc   =   '".mysql_real_escape_string($post['dec'])."' "
			." WHERE message_id = '".$post['message_id']."' AND project_id = '".$post['project_id']."' ";
		$db->query($sql);
	}	
	function GetPrivateMessageDetails($pm_id,$project_id) 
	   {
	...
	function UpdatePrivateMessage($post)
	{
		...
		$sql= " UPDATE ".PRIVATE_MESSAGE_MASTER
			." SET "
			." private_msg_desc   =   '".mysql_real_escape_string($post['dec'])."' "
...
//		return($db1->query($sql));      
	...
	function UpdatePrivateMessage_Checked($post)
	{
		...
		$sql= " UPDATE ".PRIVATE_MESSAGE_MASTER
			." SET "
			." private_msg_desc	   =   '".mysql_real_escape_string($post['private_msg_desc'])."', "
