...
if($Action == ''|| $Action == A_VIEW)
{
	...
	$ret = $project->Get_Staged_Task($_GET['project_id']);
	...
	$free = $project->Check_project($_SESSION['User_Id']); // free returns 0 if this is my first posted task and returns 1 if this is my first posted task
			if($ret->special_user == 1)		// if user is special user than no deposit required
			{
				$special_user = 1;
			}else{
	...
	$tpl->assign(array(	"T_Body"			  	=> 'publish_staged_task'. $config['tplEx'],
						"JavaScript"		  	=>  array("post_task.js"),
						"lang"				  =>  $lang,
						"stage_id"			  =>  $ret->project_id,
						"Development_List"	  =>  $Development_List,
						"sucmsg"                 =>  $SuccMsg,
						"User_Id"             =>  $_SESSION['User_Id'], 
						"user_name"           =>  $_GET['user_name'],
						"Recevier_id"         =>  md5($ret->project_posted_by),
						"amount"              =>  $inwallet, 
						...
						"en_project_title"		 =>	stripslashes($ret->project_title),
						"en_project_location"    =>	stripslashes($ret->location),
						"en_project_city"    	 =>	stripslashes($ret->city),							
						"en_project_description" =>	stripslashes($ret->project_description),
						...
						"project_file_11"		=>	$ret->project_file_1,
						"project_file_21"		=>	$ret->project_file_2,
						"project_file_31"		=>	$ret->project_file_3,

						
						"project_id"			=>	$ret->project_id,
						"project_hide" 			=> ($ret->project_hide == 1) ? 'checked' : '',
						"project_allow_bid"  	=> ($ret->project_allow_bid == 1) ? 'checked' : '',
						"premium_project"  		=> ($ret->premium_project == 1) ? 'checked' : '',
						"urgent_project"  		=> ($ret->urgent_project == 1) ? 'checked' : '',
						"project_online"  	  	=> ($ret->project_online == 1) ? 'checked' : '',							
						"selected_cat"			=>	$ret->project_category,
						"post_url"				=>	'staged_task_' . $ret->project_id . '_'. clean_url(stripslashes($ret->project_title)) . '.html',					
					));
