<?php
// Handles mail send for Notify me when new tasks related to my area of interest/expertise are posted mem_pro_notice 2
...
$cate	 = new Category();
	...
	$member_details = $cron->all_task_by_interest_subscribe();//chnages
	...
	while($db3->next_record())		// loop till last record
	{
		...
		$cron->GetprojectByCategory($member_info_cat,$membercity);
		$count = $db1->num_rows();
			
		while($db1->next_record())
		{
			$arr_related_project[$j]['project_cate']	 	= $db1->f('project_category');
			...
			$arr_related_project[$j]['dec']		      		= $db1->f('project_description');
		...
		if($count > 0)
		{
			...
			$subject_text = $sendinfo->email_subject . ' That Match Your Interest';
