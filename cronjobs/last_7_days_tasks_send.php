<?php
//Handles mail send for Notify me of all new tasks posted on Task Sonic mem_pro_notice 1 for last 7 days
...
$cate	 = new Category();
	...
	while($db3->next_record())		// loop till last record
	{
		...
		$cron->latest_tasks_by_city_7_day($membercity);
		$count = $db1->num_rows();
		...
		if($count > 0)
		{
			...
			$subject_text = $sendinfo->email_subject . ' This Past Week';
