...
$emails  = new Email();
	...
	#----------------------------------------------------------------------------------------------------
		...
		$notice_interval = $db->f('notice_interval');//Notice interval stamp used to indicate a notice has occured to avoid resend on next cron run if time condition still applies
		...
		if ($notice_status == 172800){
			$mail_template = BIDDING_END_TWO_DAY; $cron->update_notice_interval($project_id,172800); //Send 2 day remaining note
			}
		elseif ($notice_status == 86400){
			$mail_template = BIDDING_END_ONE_DAY; $cron->update_notice_interval($project_id,86400); //Send 1 day remaining note
			}
		elseif ($notice_status == 10800){
			$mail_template = BIDDING_END_THREE_HOUR; $cron->update_notice_interval($project_id,10800); //Send 3 hour remaining note
			}
		elseif ($notice_status == 3600){				
			$mail_template = BIDDING_END_ONE_HOUR; $cron->update_notice_interval($project_id,3600); //Send 1 hour remaining note
			}
		elseif ($notice_status == 9999){
			$mail_template = BIDDING_ENDED; $cron->update_bid_status($project_id); /* Set project status to 7*/ $cron->update_notice_interval($project_id,9999); // 9999 used for indicating bidding ended	
		...
		if ($notice_status == 172800 || $notice_status == 86400 || $notice_status == 10800 || $notice_status == 3600 || $notice_status == 9999){
			...
			echo '<b>Task ID: </b>' . $ret_user_task->project_id . ' - bidding ended message sent to users ' . $ret_user_task->mem_email . '<br />';
			$log = '<b>Task ID: </b>' . $ret_user_task->project_id . ' - bidding ended message sent to users ' . $ret_user_task->mem_email . '<br />';
			}else{
			echo '<b>Task ID: </b>' . $ret_user_task->project_id . ' - ' . sec2hms ($notice_status) . ' message sent to users ' . $ret_user_task->mem_email . '<br />';
			$log = '<b>Task ID: </b>' . $ret_user_task->project_id . ' - ' . sec2hms ($notice_status) . ' message sent to users ' . $ret_user_task->mem_email . '<br />';			
			}			
			// set path and name of log file (optional) 
			$cron->lfile('../cron_logs/bid_end_notification.log'); 
