...
{
	header('location: normal_members.php');
	exit();
}
...
elseif($_POST['Submit1'] == 'Send')
{
		...
		header("location: normal_members.php?mail=true");
		exit();
}
...
elseif($Action == 'Delete')	
{
  ...
  header("location: normal_members.php?del=true");
  exit();
}
elseif($Action == 'ChangeStatus')
{
	...
	header("location: normal_members.php?edit=true");
	exit();
}
...
if(!in_array($Action, array('View', 'Edit', 'Sort', 'View' , 'Send_Mail','MailSelected','Search')))
{
	...
	$tpl->assign(array("T_Body"			=>	'users_manage'. $config['tplEx'],
						...
						"Loop"			=>	$rscnt,
						//"Options"		=>	$others->ViewAll_Normal_Users(),
...
elseif($Action == 'Search' )
{
  //print_r($_POST);die;
	...
	$others->Search_Normal_Users($_POST['search_user'],$_POST['search_email']);
	$rscnt = $db->num_rows();
	...
	$tpl->assign(array("T_Body"			=>	'users_manage'. $config['tplEx'],
						...
						"user_details"	=>	$arr_user_details,
						"Loop"			=>	$rscnt,
						//"Options"		=>	$others->Search_Normal_Users($_POST['search_user']),
						"PageSize_List"	=>	$utility->fillArrayCombo($lang['PageSize_List'], $_SESSION['page_size']),						
						));
