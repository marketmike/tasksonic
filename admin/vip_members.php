...
{
	header('location: vip_members.php');
	exit();
}
...
elseif($_POST['Submit1'] == 'Send')
{
		...
		header("location: vip_members.php?mail=true");
		exit();
}
...
elseif($Action == 'Delete')	
{
  ...
  header("location: vip_members.php?del=true");
  exit();
}
elseif($Action == 'ChangeStatus')
{
	...
	header("location: vip_members.php?edit=true");
	exit();
}
...
if(!in_array($Action, array('View', 'Edit', 'Sort', 'View' , 'Send_Mail','MailSelected','Search')))
{
	...
	elseif($_GET['del'] == 'true')
	    $succMessage = "VIP User Delete sucessfully";
	elseif($_GET['edit'] == 'true')
	    $succMessage = "VIP User updated sucessfully";
	
	$others->ViewAll_VIP_Users();
	$rscnt = $db->num_rows();
	...
	$tpl->assign(array("T_Body"			=>	'users_manage'. $config['tplEx'],
						...
						"Loop"			=>	$rscnt,
						//"Options"		=>	$others->ViewAll_VIP_Users(),
...
elseif($Action == 'Search' )
{
  //print_r($_POST);die;
	...
	$others->Search_VIP_Users($_POST['search_user'],$_POST['search_email']);
	$rscnt = $db->num_rows();
	...
	$tpl->assign(array("T_Body"			=>	'users_manage'. $config['tplEx'],
						...
						"user_details"	=>	$arr_user_details,
						"Loop"			=>	$rscnt,
						//"Options"		=>	$others->Search_VIP_Users($_POST['search_user']),
						"PageSize_List"	=>	$utility->fillArrayCombo($lang['PageSize_List'], $_SESSION['page_size']),						
						));
