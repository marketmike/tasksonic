...
include_once($physical_path['Site_Lang'].'/private_message_form_user.php');
...
{
	if($HTTP_POST_FILES['attch_file']['name'] != '')
		{
			$attach_file = fileUpload($HTTP_POST_FILES['attch_file'],PRIVATE_MESSAGE);
		}
	 $en_message_desc  = badWordDetect($_POST['en_message_desc']);
	 
	 $project->Insert_Private_Msg_user($_POST,$en_message_desc,$attach_file);
		...
		$tpl2->assign(array("sender_name" 				=>	$_POST['project_creator'],
		...
		$send_to = GetEmailAddress(md5($_POST['bid_user']));
	...
	header("location: private_message_form_user.php?project_id=".$_POST['project_id']."&bid_user=".$_POST['bid_user']."&id=".$_POST['id']);
	exit();
...
elseif($Action == 'Close')
{
  if($_POST['id'] == 1)
  {
	  echo "<script type=text/javascript> opener.location.href = 'task_".$_POST['project_id']."_".clean_url($_POST['title']).".html#';";
	  echo "window.close();";
  	  echo "</script>";
  }
  elseif($_POST['id'] == 2)
  ...
  else
  {
	...
	$bid_user = ($_GET['bid_user'] ? $_GET['bid_user'] : $_POST['bid_user']);
	
	$set1 = $project->GetProject($proj_id);


	$result = $project->View_Private_Msg($proj_id,$set1->project_posted_by,$bid_user);
	...
	$tpl->assign(array(	"T_Body"			=>	'private_message_form_user'. $config['tplEx'],
