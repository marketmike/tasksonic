			 ...
			 <form name="frmproject" method="post" action="{$smarty.server.PHP_SELF}">
				...
				<div class="clear"></div>
					<div class="body_shim">
						...
						<div style="width:410px;"><strong>Task Owner's Budget:</strong> {$project_budget}<br /><strong>Task Details:</strong> {$project_description|wordwrap:1000:"\n":true}</div>
						</div><!-- /award button -->
						<div class="clear"></div>
					...
					{/if}												
						...
						<div class="field">
							...
							{else}
							{if $smarty.session.User_Id != $user_id	}<div class="buttons"><a href="#" class="footerlinkcommon2" {if $bid_status == 0 && $project_post_to != $smarty.session.User_Name }onclick="return(false);" disabled="disabled" {else}onclick="JavaScript: popupWindowURL('private_message.php?project_id={$project_id}&id=1&pop_up=true','','600','500','','true','true');" {/if}>Message</a></div>{/if}
							{/if}
							<div class="clear"></div>
	...
	<div class="page_top"></div>
        <div class="page_content">
			...
			<div class="clear"></div>
					<div class="title-links" style="width:100%;">
						<div class="form_page_text" style="text-align:center;">
							<label for="login-email-address">{$bid_type} Bids On "{$project_title|stripslashes}</label>
						</div>
					</div>			
				<div class="clear"></div>
					{if $project_status == 2}
						...
						{/if}
							...
							<div class="more_button">
									...
									<div class="clear"></div>
											...
											<div id="button_award">
												<a href="rating_user.php?project_id={$project_id}&User_type=tasker&to={$project_post_to}"  onClick="Javascript: return chk_user('{$smarty.session.User_Id}');" style='text-decoration:none;' class="negative">
												Feedback
												</a>
											</div>
											{/if}											
							...
							<div class="clear"></div>
							<div class="btm_menu">
								{if $arr_Bid[bid].Bid_Shortlist==1 && $project_status == 1}
								<a href="JavaScript: ToggleStatus_Click('{$arr_Bid[bid].bid_id}', '0');" class="">Add to Favorites</a>
								{/if}
								{if $arr_Bid[bid].Bid_Shortlist==1 && $arr_Bid[bid].Bid_Decline==1 && $project_status == 1}
								&nbsp;|&nbsp;
								{/if}
								{if $arr_Bid[bid].Bid_Decline==1 && $project_status == 1}
								<a href="#bid_list" onclick="JavaScript: popupWindowURL('undecline.php?bid_id={$arr_Bid[bid].bid_id}&provider_name={$arr_Bid[bid].User_Name}&project_id={$project_id}&id=1&pop_up=true','','645','450','','true','true');">Send to Declined</a>
								{/if}
								{if $arr_Bid[bid].Bid_Decline==0 && $project_status == 1}
					...
					{if $Tasker}
						<div id="all_bids" class="{cycle values='list_B style3, list_A style3'}" >
							...
							<div class="body_text">{if $Bid_Decline == 0}<center><span style="color:red;font-weight:bold;">Your Bid Has Been Declined. Update your bid to be reconsidered!</span></center><br /></br />{/if}<span class="title">{$lang_common.Curreny}{$Bid_Amount} to complete by {$Delivery_Time}.</span> <span>Posted on {$Bid_Time|date_format:'%B %d, %Y at %I:%M %p'}</span>
							<div class="clear"></div>		
							<span class="description">
							...
							<div class="clear"></div>
								...
								<div class="more_button">
									...
									<div id="button_award"><a href="#" class="footerlinkcommon2" onclick="JavaScript: popupWindowURL('private_message.php?project_id={$project_id}&id=1&pop_up=true','','645','500','','true','true');">Messages {if $privatecnt}({$privatecnt}){else}(0){/if}</a></div>
									{else}
									{if $project_status == 1}									
									<div id="button_award"><a href="#" class="footerlinkcommon2" onclick="JavaScript: popupWindowURL('private_message.php?project_id={$project_id}&id=1&pop_up=true','','645','500','','true','true');">Messages {if $privatecnt}({$privatecnt}){else}(0){/if}</a></div>
									{/if}
									...
									<div id="button_award">
										<a href="rating_user.php?project_id={$project_id}&User_type=task_owner&to={$project_posted_by}" style='text-decoration:none;' class="negative">
										Feedback
										</a>
									</div>
									{/if}								
	...
	<div class="page_bottom"></div>				
		...
		<div class="grid_4 omega">
			...
			<h3>Task Control Panel</h3>		
				...
				<div class="clear"></div>
				{/if}				
				{if $smarty.session.User_Name == $project_post_to && $project_status == 3 && $Escrow_Created != ''}<br /><div class="button-no" style="margin-left:70px;"><a href="mark-completed_{$project_id}.html" >Mark Completed</a></div>{/if}
				{if $smarty.session.User_Name == $project_post_to && $project_status == 3 && $Escrow_Created == ''}<br /><div class="button-no" style="margin-left:70px;"><a href="report_task_inactivity_{$project_id}.html">Report Inactivity</a></div>{/if}
				{if $smarty.session.User_Name == $project_posted_by && $project_status == 3 && $Escrow_Created != ''}<br /><div class="button-no" style="margin-left:70px;"><a href="report_task_inactivity_{$project_id}.html">Report Inactivity</a></div>{/if}				
			...
			<h3>Bid Details</h3>			
				...
				<div class="clear"></div>
				{if $smarty.session.User_Id == $user_id}
				<a href="task_{$project_id}_{$clear_title}.html" class="footerlinkcommon23"><strong>All Bids ({$count}) </strong></a>
				<div class="clear"></div>				
				<a href="shortlist_{$project_id}_{$clear_title}.html" class="footerlinkcommon23"><strong>Favorite Bids ({$shortlistcnt}) </strong></a>
				<div class="clear"></div>
				<a href="decline_{$project_id}_{$clear_title}.html" class="footerlinkcommon23"><strong>Declined Bids ({$declinecnt})</strong></a>
				<div class="clear"></div>
				{/if}
