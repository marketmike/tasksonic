        ...
        <div class="page_content">
            <div class="page_content_white">
				...
				{else}
					...
					<div class="form_page_text" style="text-align:center;">
						<label for="login-email-address">
						{if $how_short < 0}Task Posting Fee: ${$needed_to_purchase}{else}Good to Go!{/if}						
						</label>
					</div>
					</div>
					<div class="clear"></div>											
					<div class="message" style="text-align:center;padding:10px;margin:10px;{if $how_short < 0}background:#FF0000;color:#fff{else}background:#0198E1;color:#000{/if}">{if $how_short < 0}You currently have ${$amount} in your wallet which leaves you short ${$how_short}. After submitting... your task will be saved unpublished and you will be redirected to the deposit page in order to add funds to your Task Sonic Wallet. Once sufficient funds have been added you can return to the task and publish it.{if $how_short < $Premium_Fees + $Urgent_Fees + $amount} If you are planning to upgrade your task to premium (${$Premium_Fees}) or urgent (${$Urgent_Fees}), these fees will be added when the task is submitted!{/if}{else}You have ${$amount} in your Task Sonic wallet and it will cost you ${$needed_to_purchase} to post this task leaving you with ${$how_short} in your wallet. {if $how_short < $Premium_Fees + $Urgent_Fees}Please note, you may still need to add money to your Task Sonic Wallet if you are planning to upgrade your task to premium (${$Premium_Fees}) or urgent (${$Urgent_Fees}){else} You even have enough in your wallet for upgrading your task to premium (${$Premium_Fees}) and urgent (${$Urgent_Fees}) if you choose to.{/if}{/if}</div>
					<div class="clear"></div>
					<div class="note" style="text-align:center;padding:20px;">{$lang.Note1} <a href="Javascript: View_Terms()" class="footerlink"><strong>{$lang.task_Guidelines}</strong></a> {$lang.Note2}.</div>									
				<div class="clear"></div>
				{/if}
				<form method="post" action="post-new-task.html" name="frmpostproject" enctype="multipart/form-data">															
					...
					<div class="clear"></div>
					<div class="field">
						<input name="en_project_title" value="{$smarty.post.en_project_title}" type="text" maxlength="40" tabindex="1" size="50" />
					...
					<div class="clear"></div>
					<div class="field">
					<div style="line-height:40px;vertical-align:middle;margin-bottom:5px;"><input type="checkbox" name="profileaddress" value="1" tabindex="11" onClick="Javascript: return usethisaddress(this.form);" class="shorter" {$urgent_project} /> <strong>Use My Task Sonic Profile Address</strong></div>
					<input type="hidden" name="adress1" value="{$profile_address1}" /><input type="hidden" name="adress2" value="{$profile_address2}" />
					<div class="clear"></div>
						<strong>Address</strong><br /><input name="en_project_location" value="{$smarty.post.en_project_location}" type="text" maxlength="50" tabindex="2" size="50" />
					...
					<div class="clear"></div>
					<div class="field">
						<strong>City, State, Zip</strong><br /><input name="en_project_city" value="{$smarty.post.en_project_city}" type="text" maxlength="50" tabindex="2" size="50" />
					...
					<div class="clear"></div>
					<div class="field">
						<textarea name="en_project_description" title="" class="textarea" cols="50" rows="10" tabindex="3">{$smarty.post.en_project_description}</textarea>
					...
					<div class="field">
						<div style="line-height:40px;vertical-align:middle">
						<input type="checkbox" name="agree1" tabindex="12" value="1" class="shorter"  />{$lang.Note} <a href="Javascript: View_Terms()" class="footerlink"><strong>{$lang.task_Guidelines}</strong></a> and agree to them.
						</div>
					</div>
					<div class="clear"></div>
					<div class="field">
