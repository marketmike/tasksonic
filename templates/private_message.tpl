            ...
            <div class="page_content_white">
				<div class="clear"></div>
					...
					{else}
						...
						{else}
								{if $m1}
								<div class="note" style="text-align:center;padding:20px;"><h3>{$title}</h3><br />{$lang.Msg}{$project_creator}</div>
								<input class="login_txt style5" type="submit" id="btnbg" name="Submit" value="{$lang.Button_Close}" onclick="javascript : window.close();"/>
								{else}
								<div class="clear"></div>
								...
								<div class="clear"></div>					
								<div class="field">
								<div class="message_title">{$lang.Msg_Between} {$project_creator} {$lang.And} {$user_name}</div>
								<div class="clear"></div>
								{if $project_status == 3}
								...
								<div class="title-links" style="width:400px;">
									<div class="form_page_text" style="text-align:right;">
										<label for="login-email-address">Post Message To {$project_creator}</label>
									</div>
								</div>
			...
			<div class="page_top"></div>
					<div class="page_content">
						<div class="page_content_white">
							...
							<div class="title-links" style="width:100%;">
								<div class="form_page_text" style="text-align:center;">
									<label for="login-email-address">{$lang.Msg_Between} {$project_creator} {$lang.And} {$user_name}</label>
								</div>
							</div>
							<div class="clear"></div>				
									{section name=privatemsg loop=$Loop}
										<div id="message_wrapper" class="{cycle values='list_B style3, list_A style3'}">
											...
											<div class="message_body">
												...
												<div class="reply">
												{if $arr_private_msg[privatemsg].sender_login_id == $project_creator}
												<a href=#  onClick="JavaScript: ReportViolation_Click(document.frmprivatemsg,'{$arr_private_msg[privatemsg].sender_login_id}','{$user_name}');"/>Report user</a>
												{/if}
												</div>
												<div class="clear"></div>												
											</div>
