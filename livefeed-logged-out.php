...
$_SESSION["total_activity"];
    ...
    if(mysql_num_rows($result)>0){
		...
		if($activity_type == 'New Bid' || $activity_type == 'Update Bid' || $activity_type == 'Accepted' || $activity_type == 'Tasker Feedback'){
            ...
            echo '<div id="feed_wrapper">' . $img_url . '<span class="activity_desc">' . str_replace('<a', '<a onclick="return false"', $row["description"]) . '</span><div class="clear">&nbsp;</div><span class="read_more"><a href="#" onclick=popup("popUpDiv") >View Task</a></span><span class="date">' . $date . ' ago</span></div>';
        }
    }
